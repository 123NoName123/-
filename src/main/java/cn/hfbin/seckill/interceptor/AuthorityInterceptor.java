...
public class AuthorityInterceptor implements HandlerInterceptor {
    ...
    private static final String LUA_SCRIPT =  "local c = tonumber(redis.call('GET', KEYS[1]))\n" +
            "if not c then\n" +
            "    c = 1\n" +
            "    redis.call('SET', KEYS[1], c, 'EX',ARGV[1])\n" +
            "else\n" +
...
//            实现了+1，并设置有效时常
            Long count = redisService.executeLuaScript(ak, Collections.singletonList(key), LUA_SCRIPT,new Integer[]{seconds}, Long.class);
            if (count > maxCount) {
